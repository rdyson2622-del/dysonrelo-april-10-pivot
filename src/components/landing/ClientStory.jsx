...
export default function ClientStory({ label, headline, media, children, defaultOpen = false }) {
  ...
  return (
    <div className="w-full rounded-2xl mb-4 overflow-hidden" style={{ background: '#111', border: '1px solid rgba(212,175,55,0.25)' }}>
      {/* Header — text on the left, media preview on the right */}
      <div className="px-6 py-6 flex flex-col md:flex-row items-start gap-6">
        <button
          onClick={() => setOpen(!open)}
          className="flex-1 text-left flex items-start justify-between gap-4 group"
        >
      ...
      {!open && (
        <div className="relative -mt-4 h-10 pointer-events-none"
          style={{
