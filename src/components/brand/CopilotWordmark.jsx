...
export default function CopilotWordmark({ className = '', bold = false }) {
  if (bold) {
    return <span role="img" aria-label="CoPilot" className={`inline-flex items-center justify-center font-serif text-xl font-bold italic leading-none text-dyson-gold-light ${className}`}>CoPilot</span>;
  }

  return <span role="img" aria-label="CoPilot" className={`block ${className}`} style={{ backgroundColor: 'var(--dyson-gold-deep)', maskImage: `url(${WORDMARK_URL})`, WebkitMaskImage: `url(${WORDMARK_URL})`, maskMode: 'luminance', maskRepeat: 'no-repeat', WebkitMaskRepeat: 'no-repeat', maskPosition: 'center', WebkitMaskPosition: 'center', maskSize: 'contain', WebkitMaskSize: 'contain' }} />;
}