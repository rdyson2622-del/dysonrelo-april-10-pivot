...
export default function ChiefPilotPropertySearch({ title, activeProperty, isExample, onHideExample, searchLoading, searchError, onSearch, onClear, onSelectSubject, onSend, preferredClientActive, onPreferredClaim, onSaveProperty, saveStatus, selectedAgentName }) {
  ...
  const [showExamplePath, setShowExamplePath] = useState(false);
  ...
  return (
    <div className="flex min-h-full flex-col">
      ...
      <div className="mt-8 border-t border-black/10 pt-6">{!showExamplePath && <button type="button" onClick={() => setShowExamplePath(true)} className="text-sm text-dyson-gold-deep underline underline-offset-4">See example path</button>}{showExamplePath && <><ChiefPilotStoryboard property={activeProperty} agentName={selectedAgentName} onSelectSubject={onSelectSubject} /><ChiefPilotPreferredDoor active={preferredClientActive} message={gateMessage} onClaimSuccess={onPreferredClaim} /></>}</div>
