...
export default function ChiefPilotPropertySearch({ title, activeProperty, isExample, onHideExample, searchLoading, searchError, onSearch, onClear, onSelectSubject, onSend, preferredClientActive, onPreferredClaim, onSaveProperty, saveStatus }) {
  ...
  return (
    <div className="flex min-h-full flex-col">
      ...
      <form onSubmit={submit} className="mt-10 border-t border-white/10 pt-8"><label htmlFor="property-search" className="mb-3 block text-sm text-dyson-taupe">Search another property</label><div className="flex rounded-full border border-white/20 p-1.5 focus-within:border-dyson-gold"><input id="property-search" value={query} onChange={event => setQuery(event.target.value)} placeholder="Property address or listing number" className="min-w-0 flex-1 bg-transparent px-4 text-sm text-dyson-text outline-none placeholder:text-dyson-taupe/60" /><button type="submit" disabled={!query.trim() || searchLoading} className="rounded-full bg-dyson-gold px-5 py-2 text-sm text-dyson-text-dark disabled:opacity-40">{searchLoading ? 'Searching…' : 'Search'}</button></div>{searchError && <p className="mt-4 text-sm text-dyson-taupe">{searchError}</p>}</form>
