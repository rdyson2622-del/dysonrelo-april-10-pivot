...
export default function ChiefPilotStoryboard({ property, agentName, onSelectSubject }) {
  return (
    <section className="mt-10 border-t border-black/10 pt-8 text-left">
      <p className="text-[10px] tracking-[0.18em] text-dyson-gold-deep">EXAMPLE · 6228 CALLE PAVANA</p>
      <h2 className="mt-2 text-lg text-dyson-text-dark">See the full CoPilot journey</h2>
      <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{FRAMES.map(([id, type, label, line]) => <button key={id} type="button" onClick={() => onSelectSubject(id)} className="rounded-lg border border-black/20 bg-dyson-charcoal p-4 text-left hover:border-dyson-gold/50"><div className="min-h-28 rounded-md bg-dyson-black p-3"><ChiefPilotStoryboardFrame type={type} property={property} agentName={agentName} /></div><p className="mt-3 text-xs text-dyson-text">{label}</p><p className="mt-1 text-[11px] text-dyson-taupe">{line}</p></button>)}</div>
    </section>
  );
}