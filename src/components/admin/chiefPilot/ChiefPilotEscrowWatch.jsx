...
export default function ChiefPilotEscrowWatch({ title, activeProperty, escrowStub, isExample, selectedAgentName, onOpenListing, onStartEscrow }) {
  ...
  return (
    <div>
      ...
      {watch && <ol className="mt-6 divide-y divide-white/10 rounded-xl border border-black/20 bg-dyson-charcoal px-5">{watch.steps.map((step, index) => <li key={step.label} className="flex items-center justify-between gap-4 py-4 text-sm"><span className="text-dyson-text"><span className="mr-3 text-dyson-taupe">{index + 1}.</span>{step.label}</span><span className="text-xs uppercase text-dyson-taupe">{step.status}</span></li>)}</ol>}
      <div className="mt-6 flex flex-wrap items-center gap-4"><a href="tel:8583531200" className="rounded-full border border-dyson-gold/50 px-4 py-2 text-sm text-dyson-gold">Call Bob</a><button type="button" onClick={onStartEscrow} disabled={!activeProperty} className="rounded-full border border-black/20 px-4 py-2 text-sm text-dyson-text-dark disabled:opacity-40">Start watch</button></div>
    </div>
  );
}