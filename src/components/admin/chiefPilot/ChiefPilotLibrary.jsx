...
export default function ChiefPilotLibrary({ items, visitorId, isExample, onHideExample, onSearchOwn }) {
    ...
    base44.functions.invoke('getVisitorChatHistory', { visitor_id: visitorId }).then(res => {
      ...
      for (let i = 0; i < raw.length; i++) {
        if (raw[i].data?.role === 'charlie' && raw[i + 1]?.data?.role === 'user') {
          pairs.push({ id: raw[i].id, title: raw[i + 1].data.content, answer: raw[i].data.content, createdAt: raw[i].created_date });
        }
      }
        ...
        <div className="mt-8">
          ...
          <div className="mt-2 space-y-3">
            {chatHistory.map(entry => (
              <div key={entry.id} className="rounded-xl border border-black/15 bg-dyson-warm-paper px-4 py-3">
                <p className="text-sm font-medium text-dyson-text-dark">{entry.title}</p>
