  ...
  return (
    <div className="flex flex-col items-center text-center">
      ...
      <div className="mt-6 text-2xl font-medium leading-tight text-dyson-text-dark sm:text-3xl md:text-[2.7rem]">
        ...
        <p className="text-center text-[1.084rem] translate-x-[4.043rem] sm:text-[1.352rem] sm:translate-x-[6.739rem] md:text-[1.951rem] md:translate-x-[9.434rem]"><em className="italic">We Reduce</em> <em className="italic animate-stress-shake">Stress</em>!</p>
      </div>
      <div className="mt-6 flex flex-col items-start gap-2 text-left">
        <p className="text-lg text-dyson-text-dark">• We vet the top local agents with and for you in any city or state</p>
      ...
      <div className="w-full pt-6">
        ...
        <form onSubmit={event => { event.preventDefault(); search(); }} className="w-full flex justify-center"><div className="flex w-full max-w-xl items-center rounded-full border border-white/25 bg-dyson-black p-1.5 pl-5 focus-within:border-dyson-gold"><div className="relative min-w-0 flex-1"><input value={address} onChange={event => setAddress(event.target.value)} placeholder="" className="min-w-0 w-full bg-transparent text-sm text-white outline-none placeholder:text-white/55" />{!address && <div className="pointer-events-none absolute inset-0 flex items-center overflow-hidden"><div className="inline-flex animate-marquee whitespace-nowrap text-sm text-white"><span className="mr-12">{SCROLL_COPY}</span><span className="mr-12">{SCROLL_COPY}</span></div></div>}</div><button type="submit" disabled={!address.trim() || workspace.searchLoading} className="rounded-full border border-white/30 bg-dyson-charcoal px-6 py-2.5 text-sm font-semibold text-dyson-gold-light disabled:opacity-40">{workspace.searchLoading ? 'Searching…' : 'Send →'}</button></div></form>
