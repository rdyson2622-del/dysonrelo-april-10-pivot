...
const SUBJECTS = [['real-estate-solutions', 'Real Estate Solutions'], ['property-search', 'Property Search'], ['property-audit', 'Property Audit'], ['agent-vetting', 'Agent Vetting'], ['team-thread', 'Team Thread'], ['move-roadmap', 'Relocation Road Maps'], ['escrow-watch', 'Escrow Watch']];
...
export default function ChiefPilotGlobalNav() {
  ...
  let activity = [];
  const preferredClientActive = sessionStorage.getItem('chief_pilot_preferred_active') === '1';
  ...
  return (
    <section className="shrink-0 border-b border-white/10 bg-dyson-black px-4 py-5">
      ...
      <nav className="mt-5 space-y-1 border-y border-white/10 py-4">{MODES.map(([id, label]) => id === 'library' && !preferredClientActive ? <span key={id} className="block cursor-not-allowed rounded-lg px-4 py-2.5 text-sm text-dyson-taupe opacity-45">{label}</span> : <Link key={id} to={`/admin/chief-pilot?mode=${id}`} className={`block rounded-lg px-4 py-2.5 text-sm ${activeMode === id ? 'bg-white/15 text-white' : 'text-dyson-taupe hover:bg-white/5 hover:text-white'}`}>{label}</Link>)}</nav>
      <div className="pt-5"><p className="mb-2 text-[11px] text-dyson-taupe">Subjects</p><nav className="space-y-1">{SUBJECTS.map(([id, label]) => <Link key={id} to={`/admin/chief-pilot?mode=chats&subject=${id}`} className={`block border-l px-4 py-2.5 text-sm ${activeSubject === id ? 'border-dyson-gold text-white' : 'border-transparent text-dyson-taupe hover:text-white'}`}>{label}</Link>)}</nav></div>
