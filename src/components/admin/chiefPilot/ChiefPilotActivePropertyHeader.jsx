...
export default function ChiefPilotActivePropertyHeader({ property, agentName, onOpenListing }) {
  ...
  return (
    <button type="button" onClick={onOpenListing} className="sticky top-0 z-20 mb-6 flex w-full items-center gap-3 border-b border-white/10 bg-dyson-ink/95 py-3 text-left backdrop-blur">
      ...
      <div className="min-w-0 flex-1"><p className="truncate text-sm text-dyson-text">{property.fullAddress}</p><p className="mt-1 truncate text-[11px] text-dyson-taupe">{listing.status || 'Status not available'}{value ? ` · ${value}` : ''}{agentName ? ` · Agent: ${agentName}` : ''}</p></div>
