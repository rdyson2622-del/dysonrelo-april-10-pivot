    ...
    <section className="min-h-full rounded-xl bg-dyson-cream p-4 text-dyson-text-dark sm:p-5">
      ...
      <h2 className="mt-3 text-2xl font-normal leading-snug">
        {(() => {
          ...
          return <>{subject.title.slice(0, splitAt + 1)}<br /><span className="text-lg">{subject.title.slice(splitAt + 2)}</span></>;
        })()}
      </h2>
      ...
      <div className="mt-3 grid items-start gap-5 lg:grid-cols-[minmax(0,2fr)_minmax(160px,180px)]">
        <div>
          ...
          <div className="mt-3 min-h-40 max-w-2xl rounded-xl border border-white/15 bg-dyson-black p-6">
            {(messages.length || loading || error) ? <ChiefPilotConversation messages={messages} loading={loading} error={error} dark milestones={content.milestones} /> : <p className="text-sm text-dyson-taupe">Your answer and work product will appear here.</p>}
