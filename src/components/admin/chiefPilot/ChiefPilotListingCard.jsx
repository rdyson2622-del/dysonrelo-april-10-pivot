...
export default function ChiefPilotListingCard({ property, isExample, onHideExample, onClear, onAudit, onSave, onAsk, saveStatus, children }) {
  ...
  const rawListingUrl = property.listingUrl || property.sourceUrl || listing.listingUrl || listing.url || listing.detailUrl || '';
  const listingUrl = /^https?:\/\//i.test(rawListingUrl) && !/^https?:\/\/(?:www\.)?realtor\.com\/?(?:[?#].*)?$/i.test(rawListingUrl) ? rawListingUrl : '';
  return (
    <article className="overflow-hidden rounded-xl border border-white/15 bg-dyson-charcoal text-left">
      {photos.length ? <div className={`grid gap-1 ${photos.length > 1 ? 'grid-cols-2' : 'grid-cols-1'}`}>{photos.map((url, index) => <img key={url} src={url} alt={`${property.fullAddress} photo ${index + 1}`} className={`w-full object-cover ${index === 0 && photos.length > 1 ? 'row-span-2 h-64' : index === 0 ? 'h-64' : 'h-[126px]'}`} />)}</div> : listingUrl ? <a href={listingUrl} target="_blank" rel="noopener" className="flex min-h-44 items-center justify-center bg-black/30 px-6 text-center text-base text-dyson-gold underline underline-offset-4">Open full listing (new tab)</a> : <div className="flex min-h-44 items-center justify-center bg-black/30 px-6 text-center text-sm text-dyson-taupe">Exact listing link was not returned for this property.</div>}
