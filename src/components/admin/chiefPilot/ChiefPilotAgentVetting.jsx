...
export default function ChiefPilotAgentVetting({ title, activeProperty, isExample, selectedAgentName, onOpenListing, introStatus, onRequestIntro }) {
  ...
  return (
    <div>
      ...
      {activeProperty && <article className="mt-6 rounded-xl border border-black/20 bg-dyson-charcoal p-5"><p className="text-xs text-dyson-taupe">Agent-fit card · {market || 'Market pending'}</p><h3 className="mt-2 text-lg text-dyson-text">Fit for this buy</h3><div className="mt-5 grid gap-3 sm:grid-cols-2">{CHECKS.map(([label, note]) => <div key={label} className="border-l border-dyson-gold/40 pl-3"><p className="text-sm text-dyson-text">{label}</p><p className="mt-1 text-xs leading-5 text-dyson-taupe">{note}</p></div>)}</div></article>}
      <button type="button" onClick={onRequestIntro} disabled={!activeProperty || introStatus === 'saving' || introStatus === 'saved'} className="mt-6 rounded-full border border-dyson-gold-deep/60 px-4 py-2 text-sm text-dyson-text-dark disabled:opacity-40">{introStatus === 'saving' ? 'Saving intent…' : introStatus === 'saved' ? 'Vetted intro intent saved' : 'Request vetted intro'}</button>
      {introStatus === 'saved' && <p className="mt-3 text-xs text-dyson-text-dark/70">Intent recorded for review. No email or text was sent.</p>}
    </div>
  );
}