...
export default function CopilotHeroBanner({ onSearch, onOpenVoice }) {
  ...
  return (
    <section className="px-4 sm:px-8 pt-6 pb-8" style={{ background: '#0a0a0a' }}>
      <div className="max-w-5xl mx-auto flex flex-col lg:flex-row items-center gap-8 lg:gap-10">
        <div className="flex-1 space-y-6 text-center lg:text-left">
          <div className="flex items-end justify-center lg:justify-start gap-2">
            <span className="pb-1 font-serif text-2xl sm:text-3xl font-light text-white">meet</span>
            <CopilotWordmark bold className="h-9 w-28" />
          </div>

          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight leading-tight" style={{ fontFamily: 'Cormorant Garamond, serif', color: '#D4AF37' }}>
            Your Private Real Estate Copilot.
          </h1>

          <p className="text-sm sm:text-base text-white max-w-2xl mx-auto lg:mx-0 font-medium leading-relaxed">
          ...
          <form onSubmit={submit} className="max-w-2xl mx-auto lg:mx-0 pt-1">
        ...
        <div className="w-full lg:w-2/5 shrink-0 overflow-hidden rounded-2xl shadow-2xl border border-[#D4AF37]/30 bg-black p-3">
          <img src={HERO_IMAGE} alt="Luxury home with pool" className="w-full aspect-[16/9] object-cover rounded-lg" />
        </div>
      </div>
    </section>
  );
