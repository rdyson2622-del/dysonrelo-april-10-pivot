...
function JumpSearch() {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    // Accept "105" or "#105" — strip everything that isn't a digit
    const num = parseInt(query.trim().replace(/[^0-9]/g, ''));
    if (!num) return;

    // Try section registry first
    ...
    // Fall back to page registry
    const page = getPageByNumber(num);
    if (page) {
      setResult({ type: 'page', ...page, id: num });
      setNotFound(false);
      const cleanPath = page.path.replace(/:clientId|:ownerId/g, '');
      navigate(cleanPath);
      return;
    }
  ...
  return (
    <div className="px-3 pb-3 pt-2" style={{ borderTop: '1px solid #1a1a1a' }}>
      <p className="text-xs font-bold tracking-widest mb-2" style={{ color: GOLD }}>SMART SEARCH</p>
      ...
      {result && (
        <div className="mt-2 p-2 rounded-lg text-xs" style={{ background: '#1a1a1a', border: '1px solid #333' }}>
          ...
          <p className="font-mono" style={{ color: '#888' }}>{result.path}</p>
        </div>
      )}
      {notFound && (
        <p className="mt-1.5 text-xs" style={{ color: '#f87171' }}>No section found for that ID.</p>
      )}
    </div>
  );
}
