...
/**
 * BobDysonCharlieCircle — Charlie-only video circle for the Bob Dyson bio page.
 * Uses HeyGen-rendered clips from the BobDysonClip entity.
...
export default function BobDysonCharlieCircle() {
  ...
  const { data: clips = [] } = useQuery({
    queryKey: ['bobDysonClips'],
    queryFn: () => base44.entities.BobDysonClip.list(),
  });
  ...
  return (
    <>
      <button
        ...
        className="fixed top-32 right-8 z-40 w-[126px] h-[126px] md:w-36 md:h-36 transition-all hover:scale-105 active:scale-95"
      >
