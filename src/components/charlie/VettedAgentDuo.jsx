...
/**
 * VettedAgentDuo — Charlie-only video circle for the MyAgent (Vet an Agent) page.
 * Uses HeyGen-rendered clips from the VettedAgentClip entity.
...
export default function VettedAgentDuo() {
  ...
  const { data: clips = [] } = useQuery({
    queryKey: ['vettedAgentClips'],
    queryFn: () => base44.entities.VettedAgentClip.list(),
  });
  ...
  return (
    <>
      <button
        onClick={playFull}
        aria-label="Hear Charlie explain the agent vetting process"
