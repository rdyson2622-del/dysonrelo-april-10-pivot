...
// Auth pages themselves are never a valid post-login destination — landing
// back on /login (or another auth page) would create a redirect loop.
const AUTH_PATHS = ["/login", "/register", "/forgot-password", "/reset-password"];

// Resolve ?returnTo= (or ?from_url=) to a safe same-origin path, else "/portal".
//
...
export function safeReturnTo() {
  ...
  try {
    ...
    if (!path.startsWith("/") || path.startsWith("//") || path.includes("\\")) return "/portal";
    if (AUTH_PATHS.includes(url.pathname)) return "/portal";
    return path;
  } catch {
    return "/portal";
  }
