import { FileText, TrendingUp, Zap, Shield, BarChart3, Mic, Share2, Globe, Users, Send } from 'lucide-react';
...
5. HeyGen quota is monitored daily. Production pauses automatically when credits are low.`
  ...
  {
    id: 'pending-sales-campaign',
    title: '📈 Pending Sales Campaign — Listing Agent Referral Outreach',
    icon: Send,
    content: `ENTRY DATE: August 31, 2026

WHY THIS CAMPAIGN EXISTS
────────────────────────────────
Every high-value listing ($3.5M+) that goes into escrow has a listing agent who just earned the seller's trust — and whose client is a strong candidate to need a destination agent if they're relocating. This campaign turns that moment into a warm, personalized referral pitch instead of a cold call.

────────────────────────────────
THE FLOW (Admin → MLS Listing Agent Outreach)
────────────────────────────────
1. SOURCE THE LISTING: A new escrow-stage MLS listing is identified — address, agent name, brokerage, price, agent phone/email.
2. GENERATE THE LINK: Each prospect gets a unique token and a personalized preview page (/agent-preview/:token) showing their own listing back to them.
3. SEND THE EMAIL: Admin sends (or batch-sends 5 at a time, 5 seconds apart) a Gemini-drafted referral pitch explaining the 25%+ referral fee, white-glove client experience, and full transparency.
4. AGENT PREVIEWS: The agent opens their link — no login required — sees their listing, the value proposition, and can note if their client is relocating out of the area.
5. FOLLOW-UP CALL: Admin calls to close the loop and capture the client's destination if they're moving.
6. CONVERT: Interested agents can claim a permanent Relo Agent partner account directly from the preview page.

────────────────────────────────
SOURCING THE LISTING DATA
────────────────────────────────
Originally considered scraping realtor.com/homes.com for listing + agent contact data — ruled out. These sites block automated access and do not surface listing agent emails on public listing pages (same wall hit with homes.com previously).

DECISION (Aug 31, 2026): Wisdom Properties has IDX access covering all markets through their brokerage. This becomes the sourcing pipeline for daily $3.5M+ escrow listings, pending a data-access discussion with Mike West. Once confirmed, the IDX feed will be wired directly into the Listing Prospect import so agent/listing records populate automatically instead of manual paste-in.

────────────────────────────────
DELIVERABILITY: MULTI-INBOX ROTATION
────────────────────────────────
Cold B2B outreach at volume requires domain reputation hygiene. Rather than hammering one sender address, outreach emails rotate across four verified addresses on the dysonrelo.com domain via Resend:
• bob@dysonrelo.com
• partners@dysonrelo.com
• network@dysonrelo.com
• hello@dysonrelo.com
