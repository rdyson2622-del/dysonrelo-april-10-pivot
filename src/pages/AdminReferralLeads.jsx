...
import { UserPlus, CheckCircle, XCircle, Clock, TrendingUp, Users, Home, Star, Wrench, MoreHorizontal } from 'lucide-react';
...
const TYPE_STYLE = {
  relocation_client: { label: 'Relocating Client', icon: Home, color: '#D4AF37' },
  agent: { label: 'Agent', icon: Star, color: '#60a5fa' },
  vendor: { label: 'Vendor', icon: Wrench, color: '#34d399' },
  other: { label: 'Other', icon: MoreHorizontal, color: '#a3a3a3' },
};
...
export default function AdminReferralLeads() {
  ...
  const stats = useMemo(() => {
    ...
    return { total, newCount, accepted, declined, conversionRate, byType, topReferrers };
  }, [leads]);

  const filteredLeads = typeFilter === 'all' ? leads : leads.filter(l => (l.referral_type || 'other') === typeFilter);
    ...
    <div className="p-6 md:p-8 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-8">
        <button onClick={() => setTypeFilter('all')}
          ...
          style={{ background: '#111', border: `1px solid ${typeFilter === 'all' ? GOLD : 'rgba(212,175,55,0.2)'}` }}>
          <Users className="w-4 h-4 mb-2" style={{ color: GOLD }} />
          <p className="text-2xl font-black text-white">{stats.total}</p>
          <p className="text-[10px] font-bold uppercase tracking-widest text-white/40">All Types</p>
        </button>
        {Object.entries(TYPE_STYLE).map(([key, t]) => (
          <button key={key} onClick={() => setTypeFilter(key)}
            className="rounded-xl p-4 text-left transition-all"
            style={{ background: '#111', border: `1px solid ${typeFilter === key ? t.color : 'rgba(212,175,55,0.2)'}` }}>
            <t.icon className="w-4 h-4 mb-2" style={{ color: t.color }} />
            <p className="text-2xl font-black text-white">{stats.byType[key] || 0}</p>
            <p className="text-[10px] font-bold uppercase tracking-widest text-white/40">{t.label}</p>
          </button>
        ))}
      </div>
      ...
      <div className="space-y-3">
        {filteredLeads.length === 0 && <p className="text-white/50">No referral leads yet.</p>}
        {filteredLeads.map(lead => {
          const st = STATUS_STYLE[lead.status] || STATUS_STYLE.new;
          const tp = TYPE_STYLE[lead.referral_type] || TYPE_STYLE.other;
          return (
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-black tracking-wide" style={{ background: `${tp.color}22`, color: tp.color }}>
                  ...
                  <p className="text-sm text-white/60">{lead.referred_email}{lead.referred_phone ? ` · ${lead.referred_phone}` : ''}{lead.referred_company ? ` · ${lead.referred_company}` : ''}</p>
                  ...
                  <p className="text-sm mt-2" style={{ color: GOLD }}>
                    Referred by: {lead.referrer_name || 'Anonymous'}{lead.referrer_email ? ` (${lead.referrer_email})` : ''}{lead.referrer_role ? ` · ${lead.referrer_role}` : ''}
                  </p>
