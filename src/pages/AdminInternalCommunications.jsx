...
const LINKS = [
  ...
  {
    icon: MessageSquare,
    title: 'Messnd track SMS outreach and reply threads.',
  ...
  return (
    <div className="min-h-screen px-6 py-12" style={{ background: '#ede0cc' }}>
      <div className="max-w-2xl mx-auto">
        <button onClick={()istory.length > 1 ? navigate(-1) : navigate('/admin/referral-agents'))}
          className="flex items-center gap-1.5 text-xs font-bold px-3 py-2 rounded-full mb-4"
          style={{ background: '8ee', border: `1px solid ${GOLD}60`, color: '#1a1a1a' }}>
          <X className="w-3.5 h-3.5" /> Exit
        </button>

        <div classNameyle={{ background: '#0a0a0a', border: `1px solid ${GOLD}40` }}>
          <div className="flex items-center justify-center gap-2 mb-2">
            <Radio classNamack tracking-[0.2em] uppercase" style={{ color: GOLD }}>Internal Use Only</p>
          </div>
          <h1 className="text-3xl font-serif text-white text-center mb-4">Internal Company Communications</h1>
          <p className="texevery outward-facing message we send. Instead of hunting
          ...
          <div className="space-y-3">
            {LINKS.map(({ icon: Icon, title, desc, path }) => (
              <a key={path} href={path}
                ...
                style={{ background:  border: `1px solid ${GOLD}40` }}>
