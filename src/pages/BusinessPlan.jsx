...
5. HeyGen quota is monitored daily. Production pauses automatically when credits are low.`
  ...
  {
    id: 'creative-posting-sequence',
    title: '📺 Creative & Posting Sequence (SOP)',
    icon: Share2,
    content: `END-TO-END DNN BROADCAST CREATIVE & POSTING SEQUENCE — STANDARD OPERATING PROCEDURE
Entry Date: July 16, 2026

This is the canonical reference for every daily DNN broadcast — from story ingestion through social posting. Multiple shows run per day. Follow this sequence exactly every time so we never have to re-discover the steps.

══════════════════════════════════
SHARD 1 — DAILY NEWS ARTICLE PIPELINE
══════════════════════════════════

STEP 1.1 — SOURCE INGESTION (Automated / Scheduled)
• dnnNationalSourcePull runs on schedule — pulls from DnnNewsSource records (Freddie Mac PMMS, Census, FRED, etc.)
• dnnDailyArticle generates the written article using Gemini InvokeLLM with add_context_from_internet=true
• Article saved to DnnArticle entity with production_status = "new"
• Fields populated: headline, dateline, body, trigger_type, scope (local/national), audience
• Solutions generated: client_solution, agent_solution, vendor_solution

STEP 1.2 — SCRIPT GENERATION (Automated)
• dnnDailyArticle (or dnnMorningBrief) generates the Charlie + Bob broadcast script from the article
• Script saved to DnnArticle.generated_full_script (and scene-specific fields)
• production_status → "script_generated"

STEP 1.3 — ADMIN SCRIPT REVIEW (Human — DnnScriptReview page)
• Admin opens DnnScriptReview (/admin/dnn/script-review)
• Reviews generated_full_script, edits if needed → saved to edited_full_script
• Admin can set pronunciation_notes, correction_notes
• When approved: admin_approved = true, production_status → "approved_for_render"
• dnnPipelineSaveScript + dnnPipelinePullApproved handle M2M handoff to n8n/HeyGen

══════════════════════════════════
SHARD 2 — VIDEO RENDER PIPELINE
══════════════════════════════════

STEP 2.1 — HEYGEN CLIP RENDERS (Automated via n8n / HeyGen API)
• dnnMorningBroadcast orchestrates: creates DnnBroadcast record, generates script, triggers HeyGen renders per clip
• Each clip rendered via heygenRenderVideo (or combined render via dnnMorningBroadcast)
• Clips stored on DnnBroadcast.clips[] array: { role, script, heygenId, videoUrl, status }
• Render status polled by heygenCheckVideo — on completion, videoUrl saved
• ALL clips must reach status "completed" before compositing

STEP 2.2 — CLIP URL REFRESH (If needed)
• If HeyGen CDN URLs expire, run refetchBroadcastClips
• Re-downloads from HeyGen, re-uploads to Base44 storage (persistent URLs)
• Updates DnnBroadcast.clips[].videoUrl with stable Base44 URLs

STEP 2.3 — CREATOMATE STUDIO COMPOSITE (The Full Show)
• Triggered via creatomateComposite function, action: "start"
• Composites all clips onto the DNN studio background:
  - Track 1: Studio backdrop image (full screen)
  - Track 2: Static presenter headshots (frozen frames, muted)
  - Track 3: DNN opener sting → presenter clips (auto-sequenced) → DNN closer sting
  - Track 4: Name/title overlays + news pills (MARKET PULSE, RATE WATCH, MIGRATION DATA) + show text
• Render ID stored on DnnBroadcast.heygenId (reused field for Creatomate renders)
• Layout overrides via DnnBroadcast.layoutConfig (presenterWidth, positions, pillY, etc.)

STEP 2.4 — COMPOSITE POLLING & STORAGE
• creatomateComposite action: "check" polls Creatomate API for render status
• On "succeeded": downloads MP4, re-uploads to Base44 storage
• Video URL saved to DnnBroadcast.videoUrl
• VideoLibrary entry created/updated (category: "broadcast")
• On "failed": error saved to DnnBroadcast.errorMessage, render ID cleared

══════════════════════════════════
SHARD 3 — TEASER GENERATION
══════════════════════════════════

STEP 3.1 — GENERATE 5-SECOND TEASER
• Requires: DnnBroadcast.videoUrl must exist (composite complete)
• Triggered via creatomateComposite action: "generateTeaser"
• Creates a 5-second trim of the composited video
• On-screen text overlay: "WATCH THE FULL SHOW  1dnn.com/#news"
• Render ID stored on DnnBroadcast.teaserRenderId

STEP 3.2 — TEASER POLLING & STORAGE
• creatomateComposite action: "checkTeaser" polls for teaser render status
• On "succeeded": downloads MP4, re-uploads to Base44 storage
• Teaser URL saved to DnnBroadcast.teaserUrl
• teaserRenderId cleared (ready for next generation if needed)

══════════════════════════════════
SHARD 4 — SOCIAL DISTRIBUTION (Teaser + Comment Strategy)
══════════════════════════════════

STEP 4.1 — REVIEW IN DISTRIBUTION PANEL (Human — AdminShowPipeline)
• Admin opens Show Pipeline (/admin/dnn/show-pipeline)
• Selects the broadcast → DistributionPanel renders
• Admin can preview composited video and teaser before posting
• Admin can edit the post caption (editable textarea) before confirming

STEP 4.2 — POST TEASER TO LINKEDIN
• Triggered via dnnPostTeaserWithComment, channels: ["linkedin"]
• Flow:
  1. Resolves author URN (organization first, falls back to personal profile)
  2. Downloads teaser video binary
  3. Initializes LinkedIn video upload (Videos API: initializeUpload)
  4. Uploads video chunks (PUT to uploadUrl)
  5. Finalizes upload (finalizeUpload)
  6. Polls for video READY status (max 60s — teaser is tiny)
  7. Creates UGC post with teaser video + caption
  8. Posts comment with full show link (https://1dnn.com/#news)
• Distribution record saved to DnnBroadcast.distribution[]: { channel: "linkedin", status, post_id, comment_id, posted_at }

STEP 4.3 — POST TEASER TO FACEBOOK
• Triggered via dnnPostTeaserWithComment, channels: ["facebook"]
• Flow:
  1. Gets Facebook Page access token via facebook_pages connector
  2. Uploads teaser video to Page via Graph API (/page-id/videos)
  3. Posts comment with full show link
• Distribution record saved to DnnBroadcast.distribution[]: { channel: "facebook", status, post_id, comment_id, posted_at }

STEP 4.4 — SUBSCRIBER EMAIL BLAST (Optional)
• Triggered via dnnMorningEmailBlast
• Sends broadcast video link to all DnnSubscriber records
• Distribution record saved: { channel: "subscriber_email", recipient_count, posted_at }

STEP 4.5 — AGENT PRIVATE-LABEL DISTRIBUTION (Optional)
• Triggered via AdminShowPipeline → "Select Agents" button
• Sends co-branded broadcast to selected PartnerAgent records
• Tracked via AgentShowDistribution entity
• Distribution record saved: { channel: "agent_private_label", agent_id, fee, recipient_count }

══════════════════════════════════
SHARD 5 — ANALYTICS & MONITORING
══════════════════════════════════

STEP 5.1 — SOCIAL ANALYTICS POLLING
• pollSocialAnalytics runs on schedule — pulls impressions, likes, comments, shares
• LinkedIn analytics via getLinkedInPostAnalytics
• Results saved to DnnBroadcast.distribution[].analytics
• Trend tracking via previous_impressions field

STEP 5.2 — CREDIT MONITORING
• HeyGen credits monitored via heygenQuota / HeygenCreditMonitor page
• Creatomate renders billed per second (USD) via API
• Production pauses when credits are low (dnnBroadcastFailAlert sends Twilio SMS)

══════════════════════════════════
CRITICAL RULES & GOTCHAS (LEARNED THE HARD WAY)
══════════════════════════════════

1. NEVER use loop: true with trim_start/trim_duration in Creatomate — causes black box decoding.
   Use fixed-duration clips without loop attributes.

2. Creatomate volume must be percentage strings ("100%", "0%"), NOT numeric (3.0).
   Numeric volume causes silent audio in the composite.

3. Static presenter headshots (Track 2) must be MUTED (volume: "0%") — otherwise
   Bob's audio from the frozen frame overrides Charlie's active clip.

4. News pills are on Track 4 (overlays), NOT baked into the studio background.
   Pill labels: MARKET PULSE, RATE WATCH, MIGRATION DATA (HOUSING SUPPLY intentionally removed).

5. Social posting uses the TEASER (5-sec native video), NOT the full composited show.
   Full show link goes in the FIRST COMMENT, not the post body.
   This is the "Teaser + Comment" strategy — algorithm-friendly.

6. LinkedIn post caption must have the show URL as the FIRST LINE (above the headline).
   Facebook uses the same structure.

7. All social links point to https://1dnn.com/#news (deep-links to the news page).
   RoleSelector handles the hash redirect.

8. If a Creatomate render fails, clear the stale render ID (heygenId) before retrying.
   Set heygenId = "" and errorMessage = "" on the DnnBroadcast record, then re-trigger "start".

9. If a teaser needs regenerating, clear teaserUrl AND teaserRenderId, then re-trigger "generateTeaser".

10. Distribution records (DnnBroadcast.distribution[]) can be cleared via the Reset button
    in DistributionPanel to allow re-posting if a post failed or needs correction.

11. Instagram is NOT auto-posted — connector is read-only. Use manual teaser posting if needed.

12. Never re-render evergreen clips for aesthetic changes — use frontend overlays (zero HeyGen cost).
    Only re-render when the script itself changes substantively.

13. Combined rendering: Charlie's question + Bob's answer = ONE HeyGen API call (not two).
    This applies to ALL Q&A render pipelines.
...
ARTICLE PIPELINE:
• dnnNationalSourcePull — Pull from news sources
• dnnDailyArticle — Generate article + script
• dnnMorningBrief — Morning broadcast script generation
• dnnPipelinePull / dnnPipelinePullApproved — M2M script retrieval
• dnnPipelineSaveScript — Save edited scripts
• dnnPipelineUpdate — Centralized n8n callback endpoint

VIDEO RENDERING:
• dnnMorningBroadcast — Orchestrate full broadcast + HeyGen renders
• heygenRenderVideo — Individual clip render
• heygenRenderWithAudio — Clip render with custom audio
• heygenCheckVideo — Poll HeyGen render status
• refetchBroadcastClips — Refresh expired clip URLs
• creatomateComposite — Studio composite + teaser (actions: start, check, generateTeaser, checkTeaser)

SOCIAL DISTRIBUTION:
• dnnPostTeaserWithComment — Post teaser to LinkedIn + Facebook (with link comment)
• dnnMorningEmailBlast — Email broadcast to subscribers
• dnnAutoDistributeBroadcast — Automated multi-channel distribution
• postToLinkedInV2 — Direct LinkedIn posting (legacy/alternative)
• postBroadcastToFacebook — Direct Facebook posting (legacy/alternative)
• postBroadcastToInstagram — Instagram posting

ANALYTICS & MONITORING:
• pollSocialAnalytics — Poll social engagement metrics
• getLinkedInPostAnalytics — LinkedIn-specific analytics
• heygenQuota — Credit monitoring
• dnnBroadcastFailAlert — Twilio SMS alert on render failure
• notifyAdmin — General admin notification

══════════════════════════════════
DAILY OPERATIONS CHECKLIST
══════════════════════════════════

□ Morning: dnnNationalSourcePull + dnnDailyArticle run (scheduled)
□ Review generated scripts in DnnScriptReview
□ Approve scripts for rendering
□ Monitor HeyGen clip renders (dnnMorningBroadcast)
□ All clips completed? → Trigger creatomateComposite "start"
□ Composite complete? → Verify videoUrl on DnnBroadcast
□ Generate teaser (creatomateComposite "generateTeaser")
□ Teaser ready? → Open Show Pipeline → DistributionPanel
□ Review/edit caption → Post to LinkedIn
□ Review/edit caption → Post to Facebook
□ Optional: Email blast to subscribers
□ Optional: Agent private-label distribution
□ Monitor analytics via pollSocialAnalytics (scheduled)

This SOP is the single source of truth. If a step changes, update THIS document — not a chat thread.`
  },
  {
