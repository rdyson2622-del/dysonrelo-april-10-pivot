...
export default function AdminAgentVetting() {
  ...
  const exportReferralCSV = async () => {
    setExporting(true);
    try {
      const filter = referralFilter === 'all' ? {} : { list_name: referralFilter };
      const allAgents = await base44.entities.ReferralAgentList.filter(filter, '-created_date', 10000);
      ...
      const rows = allAgents.map(a => headers.map(h => escape(a[h])).join(','));
