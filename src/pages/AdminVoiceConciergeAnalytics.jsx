...
import { Phone, Clock, DollarSign, Map, MessageSquare, X } from 'lucide-react';
...
export default function AdminVoiceConciergeAnalytics() {
  ...
  return (
    <div className="min-h-screen p-6" style={{ background: '#0a0a0a' }}>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr style={{ background: '#111' }}>
              {['Caller', 'Started', 'Duration', 'Turns', 'Roadmap', 'Est. Cost', 'Transcript'].map((h) => (
                ...
                <tr key={s.id} style={{ borderTop: '1px solid rgba(212,175,55,0.1)' }}>
                  ...
                  <td className="px-4 py-2.5">
                    {s.transcript?.length > 0 ? (
                      ...
                      >
                        <MessageSquare className="w-3 h-3" /> View ({s.transcript.length})
                      </button>
                    ) : '—'}
                  </td>
                </tr>
      ...
      {openTranscript && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ background: 'rgba(0,0,0,0.6)' }} onClick={() => setOpenTranscript(null)}>
          <div
          ...
          >
            <div className="shrink-0 flex items-center justify-between px-5 py-4" style={{ borderBottom: '1px solid rgba(212,175,55,0.2)' }}>
              <p className="text-xs font-black tracking-widest uppercase" style={{ color: GOLD }}>
                {openTranscript.user_name || openTranscript.user_email || 'Caller'} · Transcript
              </p>
              <button onClick={() => setOpenTranscript(null)} className="w-6 h-6 flex items-center justify-center rounded-full"
                style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(212,175,55,0.4)', color: GOLD }}>
                <X className="w-3 h-3" />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-5 space-y-3">
              {(openTranscript.transcript || []).map((entry, i) => (
                <div key={i} className={`flex ${entry.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className="max-w-[85%] rounded-xl px-3 py-2 text-sm"
                    ...
                    <p className="text-[10px] font-bold mb-0.5" style={{ color: entry.role === 'user' ? GOLD : '#aaa' }}>
                      {entry.role === 'user' ? 'Caller' : 'Charlie'}
                    </p>
                    {entry.text}
                  </div>
                </div>
