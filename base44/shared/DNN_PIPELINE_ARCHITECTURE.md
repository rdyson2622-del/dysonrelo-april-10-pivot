...
## Multi-Scene Format (Charlie → Bob → Charlie)

Every daily show is a **3-scene sequence** rendered as a **single HeyGen job**:

| Scene | Avatar | Script Field | Description |
|-------|--------|-------------|-------------|
| 1 | Charlie | `intro_script` | Opens the show, introduces headlines |
| 2 | Bob | `content_script` | Explains the main story / market context |
| 3 | Charlie | `outro_script` | Closes the show, CTA |

### Gemini Output Constraint
Gemini MUST output structured JSON with 3 separate scene keys:
```json
{
  "intro_script":   "<Charlie spoken script>",
  "content_script": "<Bob spoken script>",
  "outro_script":   "<Charlie spoken script>",
  "captions":       "<short-form social copy + hashtags>",
  "sms_text":       "<short snippet for text broadcast>"
}
```

### HeyGen Dispatch Rule (CRITICAL)
n8n Workflow 1 must bundle all 3 scenes into a **SINGLE** HeyGen POST request:
- Use HeyGen's multi-scene `video_inputs` array, OR map to a pre-made 3-scene HeyGen template
- **DO NOT** fire 3 separate API calls
- HeyGen returns **ONE** `heygen_video_id` for the entire stitched show
- HeyGen sends **ONE** completion webhook to Workflow 2 when the full MP4 is ready

This guarantees: 1 dispatch → 1 job id → 1 webhook → 1 MP4 → 1 distribution trigger.
...
| `intro_script` | text | Stage 1 | Scene 1 — Charlie intro (Gemini output) |
| `content_script` | text | Stage 1 | Scene 2 — Bob content (Gemini output) |
| `outro_script` | text | Stage 1 | Scene 3 — Charlie outro (Gemini output) |
| `script` | text | Stage 1 | Combined full script (legacy/backward compat) |
| `heygenId` | string | Stage 1 | HeyGen video job id — single multi-scene render (set by W1) |
| `videoUrl` | string (CDN) | Stage 2 | Final stitched MP4 URL (set by W2 after cloud upload) |
...
```
draft (Scripting)
  ↓  n8n W1 dispatches 3-scene bundle to HeyGen, calls dnnRenderDispatched
rendering (HeyGen rendering all 3 scenes in background)
  ↓  HeyGen calls n8n W2 webhook with single stitched MP4, W2 uploads to cloud, calls n8nBroadcastCallback
...
- **Called by**: n8n Workflow 1 (after single multi-scene HeyGen dispatch)
...
- **Payload**:
  ```json
  {
    ...
    "script": "<optional combined full script>",
    "pipeline_secret": "<secret>"
  }
  ```
- **Action**: Sets `status="rendering"`, stores `heygenId` + 3 scene scripts

### 2. `n8nBroadcastCallback` — Stage 2 callback (EXISTING, reused)
- **URL**: `https://1dnn.com/functions/n8nBroadcastCallback`
- **Called by**: n8n Workflow 2 (after cloud upload of stitched MP4)
...
2. **Gemini node**: Generate news script as structured JSON with 3 scene keys:
   - `intro_script` (Charlie), `content_script` (Bob), `outro_script` (Charlie)
   - Plus `captions` and `sms_text` for distribution
3. **HeyGen node**: POST `/v2/video/generate` with ALL 3 scenes bundled into a SINGLE request:
   - Use multi-scene `video_inputs` array (Scene 1: Charlie avatar, Scene 2: Bob avatar, Scene 3: Charlie avatar)
   - OR map to a pre-made 3-scene HeyGen template ID
   - Include `webhook_url` parameter → n8n W2 listener URL
   - **DO NOT fire 3 separate API calls**
4. **HTTP node**: POST to `https://1dnn.com/functions/dnnRenderDispatched` with `{ broadcast_id, heygen_video_id, intro_script, content_script, outro_script }` + `x-pipeline-secret` header
...
1. **Trigger**: n8n Webhook node (listens for HeyGen `video_status.completed`)
   ...
   - Receives ONE webhook for the full stitched 3-scene MP4
...
| `HEYGEN_API_KEY` | n8n W1 | HeyGen video generation (multi-scene) |
| `GEMINI_API_KEY` | n8n W1 | Script generation (3-scene JSON output) |
