    ...
    if (action === 'check') {
      ...
      for (const rec of rendering) {
        ...
        if (rec.clips?.length > 0) {
          ...
          // Stitching is NOT auto-triggered — an admin must click "Start
          // Stitching" in the pipeline UI. This avoids firing extra HeyGen
          // renders (and burning credits) from a background poll.
