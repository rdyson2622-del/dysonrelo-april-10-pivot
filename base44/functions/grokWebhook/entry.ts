...
/**
 * grokWebhook — Simple JSON API gateway for Grok / external AI clients.
 *
 * Replaces MCP server. No OAuth, no token exchange — just an API key.
 *
 * USAGE:
 *   POST https://1dnn.com/api/functions/grokWebhook
 ...
 *   GET (no key)       → 401
 *
 * All operations run as the service role (admin-level) since the API key
