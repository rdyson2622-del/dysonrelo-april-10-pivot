...
/**
 * portalLeadInRender — renders the PortalLeadInClip Charlie/Bob DNN strategy dialogue.
 *
 * Charlie (avatar) introduces Bob → Bob (talking photo) explains DNN strategy → Charlie closes.
 *
 * Actions (POST body):
  ...
  try {
    ...
    const checkRender = async (clip, role) => {
      ...
      if (status === 'completed') {
        ...
        const file = new File([buf], `portalleadin_${clip.id}_${role}.mp4`, { type: 'video/mp4' });
