...
import { mapWebhookEventToMilestone, upsertEscrowMilestone, resolveBrokerageId } from '../../shared/boldtrailSync.ts';
...
export default async function(req) {
  try {
    ...
    const brokerageId = await resolveBrokerageId(base44, event.brokerage_id);
    let created = 0, updated = 0, skipped = 0;
    for (const evt of events) {
      try {
        const milestone = mapWebhookEventToMilestone(evt, evt.client_id, brokerageId);
