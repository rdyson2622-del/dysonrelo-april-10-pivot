...
<body style="margin:0;padding:0;background:#0a0a0a;font-family:'Georgia',serif;">
  <div style="max-width:600px;margin:0 auto;background:#0a0a0a;padding:40px 32px;">
    ...
    // One activity copy to Bob (not N× Bcc)
    try {
      await base44.asServiceRole.integrations.Core.SendEmail({
        to: 'rdyson2622@gmail.com',
        subject: `DNN media activity: ${comm.subject}`,
        body: `<p>Communication blast complete.</p><p><strong>${comm.subject}</strong></p><p>Sent: ${sent} · Failed: ${failed} · Targets: ${subs.length}</p>`,
        from_name: 'Bob Dyson — DNN',
      });
    } catch (_) { /* non-fatal */ }
