...
} from '../../shared/claudeLibraryDrive.ts';

/**
 * claudeLibraryProvisionDocs — admin-only.
  ...
  try {
    ...
    for (const node of nodes) {
      ...
      const seedText = node.content || node.summary
        ...
        : `${node.title}\n\n(This document is the canonical content for the "${node.title}" node in the Claude AI Agent Library. Edit this doc — then run sync to update Base44.)`;
    ...
    let masterText = "CLAUDE AI AGENT LIBRARY — MASTER OVERVIEW\n\n";
    masterText += "This document mirrors the Base44 Claude AI Agent Library Flow Chart.\n";
